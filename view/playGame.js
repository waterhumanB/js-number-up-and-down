...
async function evaluateGuess(answer, randomNum, inputValue, max) {
...
async function playGame(min,max,count) {
  ...
  while ( answer.length < count){
    ...
    result = await validateInput(inputValue,randomValue,answer,max)
    ...
    if(await evaluateGuess(answer,randomValue ,inputValue,max)) {
      break
    }
  }
